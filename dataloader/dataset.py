...
class ImageOcc_NuScenes(data.Dataset):
    def __init__(self, data_path, occ_path, imageset='train', label_mapping="nuscenes.yaml", nusc=None, interval=[0.2, 0.2, 0.2]):
        with open(imageset, 'rb') as f:
            data = pickle.load(f)
        
        # do we still need label mapping
        with open(label_mapping, 'r') as stream:
            nuscenesyaml = yaml.safe_load(stream)
        self.learning_map = nuscenesyaml['learning_map']
    ...
    def __getitem__(self, index):
        ...
        sample_meta = self.nusc.get('sample', info['token'])
        scene_token = sample_meta['scene_token']
        lidar_token = sample_meta['data']['LIDAR_TOP']

        rel_path = 'scene_{0}/occupancy/{1}.npy'.format(scene_token, lidar_token)
        #  [z y x cls]
        occ = np.load(os.path.join(self.occ_path, rel_path))
        label = np.where(occ[:, 3:] == 0, 17, occ[:, 3:])
        occ = np.concatenate([occ[:, :3], label], axis=1)
        # xyz, labels = self.process_occ(occ)
        
        data_tuple = (imgs, img_metas, occ)
        return data_tuple
